...
export default defineNuxtRouteMiddleware(async (to) => {
  ...
  if (import.meta.server) {
    ...
    if (!cookieHeader || !cookieHeader.includes('refresh_token=')) {
      if (import.meta.dev) {
        // Dev mode: skip SSR auth, let client-side handle it
        return
      }
    ...
    try {
      ...
      // Backend rotates the refresh token on each call, so we must capture
      // the Set-Cookie header and forward it to the browser
